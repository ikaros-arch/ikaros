    ...
    <Grid size={{ xs: 12, sm: expanded ? 12 : 6, md: expanded ? 12 : 4 }}>    
      <Card variant="outlined" sx={{ marginBottom: 2, width: '100%', gridRowEnd: expanded ? 'span 2' : undefined }}>
        ...
        <CardActions>
          ...
          <IconButton aria-label="edit" size="small" onClick={() => handleEdit(resourceType)}>
            <Edit />
          </IconButton>
          <IconButton aria-label="delete" size="small" onClick={() => handleDelete(resourceType)}>
            <Delete />
          </IconButton>        
        </CardActions>
        <Collapse in={expanded} timeout="auto" unmountOnExit>
                      {resourceType === 'images' && (
                        <CardMedia
                          component="img"
                          height="500"
                          image={`https://${domain}/resources/redirect?url=${link}`}
                          alt="Image preview"
                      ...
                      {!(resourceType === 'images' || link.toLowerCase().endsWith('.pdf')) && (
                        <iframe
                          src={`https://${domain}/resources/redirect?url=${link}`}
                          width="100%"
                          height="500"
                          frameBorder="0"
