...
const SurveyGrid = () => {
  console.log("Render SurveyGrid")
  const currData = useStore(state => state.currGrid);
  ...
  useEffect(() => {
    if (currData) {
      const { area, geom, geometry, teamleader, walkers, updated_by, ...scapeData } = currData;
        ...
        <Grid container spacing={1} padding={1} sx={{ paddingTop: '22px' }}>
          ...
          <Grid size={{ xs: 12 }} order={{ xs: 5, sm: 4 }}>
            <Typography gutterBottom variant="body" component="p">
              No grid selected. Add new or browse through existing.
            </Typography>
          </Grid>
        </Grid>
      </>
      ...
      <Grid container spacing={1} padding={1} sx={{ paddingTop: '22px' }}>
        ...
        <Grid size={{ xs: 12, sm: 8 }} order={{ xs: 6 }}>
          <Autocomplete
            ...
            onChange={(event, newValue) => handleMultiAutocompleteChange(event, newValue, 'collected_by_uuid', currData, setCurrData)}
