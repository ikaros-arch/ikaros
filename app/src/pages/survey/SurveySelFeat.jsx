...
// Set scape_uuid for update on change in either map or dropdown
  ...
  const handlePress = async (target) => {
    if (target === 'select') {
      const updateData = await makeRequest('post', `edit_feature`, data, 'Prefer: return=representation');
      const getData = await makeRequest('get', `view_feature?uuid=eq.${updateData[0].uuid}`, {}, {});
      setCurrData(getData[0]);
      console.log(getData);
    }
    navigate('/surv_feature');
  };
    ...
    <Grid container spacing={1} padding={1} sx={{ paddingTop: '22px' }}>
      ...
      <Grid size={{ xs: 3, sm: 3, md: 3 }} order={{ xs: 1 }}>
          <TextField
            ...
            name="feature_no"
            fullWidth              
            value={data.feature_no || null}
            onChange={(event) => setData({ ...data, 'feature_no': event.target.value })} 
          />   
      </Grid>                                          
