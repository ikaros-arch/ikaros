...
const SurveySelLine = () => {
  console.log("Render AddSurveyLine")
...
// Set tract_uuid for update on change in either map or dropdown
  useEffect(() => {
    setData({ ...data, 'walker_uuid': activeActor?.uuid, 'updated_by_uuid': activeActor?.uuid })
  }, [activeActor]);
...
// Set tract_uuid for update on change in either map or dropdown
  ...
  const handlePress = async (target) => {
    if (target === 'select') {
      const updateData = await makeRequest('post', `edit_line`, data, 'Prefer: return=representation');
      const getData = await makeRequest('get', `view_line?uuid=eq.${updateData[0].uuid}`, {}, {});
      setCurrLine(getData[0]);
      console.log(getData);
    }
    navigate('/surv_line');
  };
    ...
    <Grid container spacing={1} padding={1} sx={{ paddingTop: '22px' }}>
      ...
      <Grid size={{ xs: 3, sm: 3, md: 3 }} order={{ xs: 1 }}>
        <Autocomplete
          ...
          id="line"
          options={lines}
          onChange={(event, newValue) => setData({ ...data, 'line_no': newValue?.value })}            
